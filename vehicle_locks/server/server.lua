-- server.lua | Personal Vehicle System v5.5.0
...
-- Standard lock toggle
RegisterNetEvent("PersonalVehicle:RequestLockToggle", function(netId, shouldLock, x, y, z, suppressSound)
    local src      = source
    local netIdStr = tostring(netId)
    if personalVehicles[netIdStr] ~= src then return end
    local now = GetGameTimer()
    ...
    if not suppressSound and shouldLock ~= nil and x then
        local sound = shouldLock and Config.SoundFileNameLock or Config.SoundFileNameUnlock
        TriggerClientEvent("PersonalVehicle:PlaySound", -1,
            x, y, z, Config.SoundReach, sound, Config.SoundVolume,
            Config.SoundReverbEnabled, Config.SoundReverbMaxWet, Config.SoundReverbStart)
    end
end)
...
-- Aircraft lock
RegisterNetEvent("PersonalVehicle:RequestAircraftLock", function(netId, shouldLock, x, y, z)
    local src      = source
    local netIdStr = tostring(netId)
    if personalVehicles[netIdStr] ~= src then return end
    local now = GetGameTimer()
    ...
    if shouldLock ~= nil and x then
        local sound = shouldLock and Config.AircraftLockSound or Config.AircraftUnlockSound
        TriggerClientEvent("PersonalVehicle:PlaySound", -1,
            x, y, z, Config.AircraftSoundReach, sound, Config.AircraftSoundVolume,
            Config.AircraftSoundReverbEnabled, 0, 0)
    end
end)
...
RegisterNetEvent("PersonalVehicle:RequestFlash", function(netId, count, speed, x, y, z)
    ...
    TriggerClientEvent("PersonalVehicle:DoFlash", -1, tostring(netId), count or 1, speed or 150, x, y, z)
end)

RegisterNetEvent("PersonalVehicle:StartAlarm", function(netId, x, y, z)
    if not netId or not x then return end
    local netIdStr = tostring(netId)
    TriggerClientEvent("PersonalVehicle:TriggerAlarm", -1, netIdStr, x, y, z)
    local owner = personalVehicles[netIdStr]
    if owner then
...
RegisterNetEvent("PersonalVehicle:StopAlarm", function(netId, x, y, z)
    if not netId or not x then return end
    local netIdStr = tostring(netId)
    TriggerClientEvent("PersonalVehicle:CancelAlarm", -1, netIdStr)
    local owner = personalVehicles[netIdStr]
    if owner then
...
RegisterNetEvent("PersonalVehicle:RequestSync", function()
    TriggerClientEvent("PersonalVehicle:UpdateOwnership", source, personalVehicles)
end)
