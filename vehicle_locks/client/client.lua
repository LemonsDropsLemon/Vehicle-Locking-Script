...
-- Exterior Lock Toggle (double-tap E)
local function ToggleLock()
    ...
    if shouldLock then
        ...
        if not isMoto then RequestFlash(netId, 1, 150, coords) end
        ...
        SetTimeout(250, function()
            if not isMoto and DoesEntityExist(veh) then
...
local function InteriorToggleLock()
    ...
    local veh, vehType = GetOwnedVehicle()
    if not veh or vehType == "moto" then return end
    ...
    if Config.EnableSound then
        local soundFile = shouldLock and Config.InteriorLockSoundLock or Config.InteriorLockSoundUnlock
        SendNUIMessage({
...
RegisterNetEvent("PersonalVehicle:PlayMuffledSound", function(x, y, z, maxDist, soundFile, volume, muffleFreq)
    if not Config.EnableSound then return end
    local ped = PlayerPedId()
    local pc  = GetEntityCoords(ped)
    if #(pc - vector3(x, y, z)) > maxDist then return end
    ...
    if IsPedInAnyVehicle(ped, false) then
        local veh = GetVehiclePedIsIn(ped, false)
        if #(GetEntityCoords(veh) - vector3(x, y, z)) < 3.0 then
            SendNUIMessage({
                transactionType = "playDirectSound",
                file            = soundFile,
                volume          = Config.InteriorLockSoundVolume,
