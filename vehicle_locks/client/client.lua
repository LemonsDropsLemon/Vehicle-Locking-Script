-- client.lua | Personal Vehicle System v5.5.0
...
local ANIM_MOTO_CLIP = "fixing_a_player"
...
local function ToggleLock()
    ...
    if isMoto and Config.MotorcycleRequireFacing
    and not IsFacingVehicle(ped, veh, Config.MotorcycleFacingAngle) then
    ...
    if GetVehiclePedIsIn(ped, false) ~= veh and not EnsureNetworkControl(veh) then
        local ev = isAircraft and "PersonalVehicle:RequestAircraftLock" or "PersonalVehicle:RequestLockToggle"
        TriggerServerEvent(ev, netId, shouldLock, coords.x, coords.y, coords.z, suppressSound)
        cooldown = false
        return
    end
    ...
    if shouldLock then
        ...
        if not isMoto and not isAircraft then RequestFlash(netId, 1, 150, coords) end
    else
        ...
        if not isMoto and not isAircraft then
            SetTimeout(250, function()
                if DoesEntityExist(veh) then RequestFlash(netId, 2, 150, GetEntityCoords(veh)) end
            end)
        end
    end

    notify(Config.ShowVehicleName
        and string.format(
            shouldLock and Config.Messages.VehicleLockedWithName or Config.Messages.VehicleUnlockedWithName,
            GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(veh))))
        or (shouldLock and Config.Messages.VehicleLocked or Config.Messages.VehicleUnlocked))

    if Config.HudEnabled then
        SendNUIMessage({ transactionType = "showLockHud", locked = shouldLock, displayTime = Config.HudDisplayTime })
    end
...
local function InteriorToggleLock()
    ...
    if Config.EnableSound then
        local soundFile = isAircraft
            and (shouldLock and Config.AircraftLockSound   or Config.AircraftUnlockSound)
            or  (shouldLock and Config.InteriorLockSoundLock or Config.InteriorLockSoundUnlock)
        local volume = isAircraft and Config.AircraftSoundVolume or Config.InteriorLockSoundVolume
        SendNUIMessage({ transactionType = "playDirectSound", file = soundFile, volume = volume })
    end
        ...
        if veh ~= currentVehicle then
            if veh == 0 and myOwnedNetId and DoesEntityExist(currentVehicle) then
                ...
                and GetVehicleDoorLockStatus(currentVehicle) >= 2 then
                    ...
                    if Config.EnableSound then
                        local soundFile = wasAircraft and Config.AircraftUnlockSound or Config.InteriorLockSoundUnlock
                        local volume    = wasAircraft and Config.AircraftSoundVolume  or Config.InteriorLockSoundVolume
                        SendNUIMessage({ transactionType = "playDirectSound", file = soundFile, volume = volume })
                    end
                    if wasAircraft then
...
RegisterNetEvent("PersonalVehicle:PlaySound", function(x, y, z, maxDist, soundFile, volume, reverbEnabled, reverbMaxWet, reverbStart)
    ...
    SendNUIMessage({
        ...
        reverbMaxWet    = reverbMaxWet or 0,
        reverbStart     = reverbStart  or 0,
        srcX            = x,
        srcY            = y,
        playerX         = pc.x,
        playerY         = pc.y,
...
RegisterNetEvent("PersonalVehicle:PlayMuffledSound", function(x, y, z, maxDist, soundFile, volume, muffleFreq, srcPlayerId)
    ...
    if IsPedInAnyVehicle(ped, false) then
        ...
        if #(GetEntityCoords(veh) - vector3(x, y, z)) < halfSize + 1.0 then
            SendNUIMessage({ transactionType = "playDirectSound", file = soundFile, volume = Config.InteriorLockSoundVolume })
            return
        end
    end
...
-- Request full ownership sync on resource (re)start

AddEventHandler("onClientResourceStart", function(resourceName)
    if GetCurrentResourceName() ~= resourceName then return end
    TriggerServerEvent("PersonalVehicle:RequestSync")
end)