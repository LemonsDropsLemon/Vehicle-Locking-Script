...
(function () {
    ...
    // muffleFreq: optional Hz value inserts a per-chain lowpass before masterFilter
    function buildChain(buf, loop, useReverb, muffleFreq) {
        ...
        var chainFilter = null;

        source.connect(dryGain);
        dryGain.connect(panNode);

        if (muffleFreq) {
            chainFilter = ctx.createBiquadFilter();
            chainFilter.type            = 'lowpass';
            chainFilter.frequency.value = muffleFreq;
            chainFilter.Q.value         = 0.7;
            panNode.connect(chainFilter);
            chainFilter.connect(masterFilter);
        } else {
        ...
        return { source: source, gainNode: dryGain, panNode: panNode, chainFilter: chainFilter, wetGain: wetGain };
    }
    ...
    function disconnectChain(nodes) {
        ...
        if (nodes.chainFilter) { try { nodes.chainFilter.disconnect(); } catch (e) {} }
        if (nodes.wetGain)     { try { nodes.wetGain.disconnect();     } catch (e) {} }
    }
    ...
    // Spatial one-shot (optionally muffled via per-chain lowpass)
    function playSound(data) {
        ...
        loadBuffer(data.file).then(function (buf) {
            ...
            var nodes = buildChain(buf, false, rv.useReverb, data.muffleFreq || null);
    ...
    // Non-spatial one-shot full volume, centre pan, no reverb (interior sounds)
    function playDirectSound(data) {
        tryResume();
        loadBuffer(data.file).then(function (buf) {
            var nodes = buildChain(buf, false, false, null);
            nodes.gainNode.gain.value = data.volume || 1.0;
            nodes.panNode.pan.value   = 0;
            nodes.source.start();
            nodes.source.onended = function () { disconnectChain(nodes); };
        }).catch(function (e) { console.error('[PV Audio] playDirectSound:', e); });
    }
    ...
    function startAlarm(data) {
        ...
        loadBuffer(data.file).then(function (buf) {
            if (activeAlarms[data.key]) return;
            var sp    = computeSpatial(data.srcX, data.srcY, data.playerX, data.playerY, data.fwdX, data.fwdY, data.maxDist, rv.reverbMaxWet, rv.reverbStart);
            var nodes = buildChain(buf, true, rv.useReverb, null);
            applyInitial(nodes, data.volume, sp);
            nodes.source.start();
